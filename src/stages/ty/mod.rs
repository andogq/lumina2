...
impl Hir {
    fn visit(&self, visitor: &mut impl HirVisitor) {
        self.functions.iter().for_each(|decl| {
            visitor.visit_function_declaration(
                decl.parameters.iter().map(|(_, ty)| ty.clone()).collect(),
        ...
        self.exprs.iter().enumerate().for_each(|(id, expr)| {
            ...
            match expr {
                ...
                Expr::Variable(variable) => {
                    visitor.visit_variable(id, variable, &self.bindings[&variable.binding])
                }
            }
        });
    }
